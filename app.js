...
function startPreviewAnimation() {
  ...
  function animate(timestamp) {
    ...
    const currentImage = images[previewCurrentImageIndex];
    const imageDuration = currentImage.duration * 1000; // 초를 밀리초로

    if (!imageStartTime) imageStartTime = timestamp;
    const elapsed = timestamp - imageStartTime;
    const progress = Math.min(elapsed / imageDuration, 1.0); // 0~1
    ...
    if (progress >= 1.0) {
      ...
      if (previewCurrentImageIndex >= images.length) {
        previewCurrentImageIndex = 0; // 처음부터 다시
      }
      imageStartTime = timestamp; // 새 이미지 시작 시간
    }
